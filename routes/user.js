...
exports.getReset = (req, res, next) => {
	...
	var now = new Date();
	db.selectUserAccountByResetToken(req.params.token, now, (err, user) => {
		if (err) {
			return next(err);
		}
		if (user === null) {
	...
	db.selectUserAccountByResetToken(req.params.token, now, (err, user) => {
		if (err) {
			...
			user.generatePasswordHash(req.body.password, (err, hash) => {
				...
				db.updatetUserAccountPasswordViaReset(user, (err) => {
					if (err) {
						return next(err);
					}
					req.logIn(user, (err) => {
						...
						const mailOptions = {
							to: user.email,
							from: 'noreply@myteams.herokuapp.com',
							subject: 'Your myTeam password has been changed',
...
exports.postForgot = (req, res, next) => {
	...
	crypto.randomBytes(16, (err, buf) => {
		...
		db.selectUserAccountByEmail(req.body.email, (err, user) => {
			if (err) {
				next(err);
			}
			if (user === null) {
				...
				db.updatetUserAccountPasswordReset(user, (err) => {
					if (err) {
						next(err);
					}
					...
					var mailOptions = {
						to: user.email,
						from: 'noreply@myteams.herokuapp.com',
						subject: 'Reset your password on myTeam',
