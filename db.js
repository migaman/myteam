...
module.exports = {
	...
	insertPlayer: function (player, cb) {

		var sql = "INSERT INTO mt_player(firstname, lastname, playernr) VALUES ($1, $2, $3) RETURNING idplayer";
		pool.query(sql, [player.lastname, player.firstname, player.playerno], (err, rs) => {
			if (err) {
				cb(err);
			}
			else {
				var idplayer = rs.rows[0].idplayer;
				cb(err, idplayer);
			}
